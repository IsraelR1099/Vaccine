...
def test_response(data, form_data, url, method, payload, false_payload, input_tag, user_agent):
    """
    ...
    response = send_request(
        action_url, data, method, user_agent
    )
    if response is None:
...
def boolean_based(form_data, url, method, user_agent, output_file):
    """
        ...
        for input_tag in form_data["inputs"]:
            ...
            for tag in form_data["inputs"]:
                if tag["type"] == "hidden" or tag["value"]:
                    ...
                    data[tag["name"]] = "test"
            if test_response(data, form_data, url,
                             method, payload, false_payload,
                             input_tag, user_agent):
